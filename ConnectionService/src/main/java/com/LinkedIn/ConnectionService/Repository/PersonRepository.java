...
public interface PersonRepository extends Neo4jRepository<Person, Long> {
    ...
    @Query("MATCH (a:Person {userId:$userId})-[:CONNECTED_TO]-(b:Person) RETURN b")
    ...
    @Query("MATCH (p1:Person {userId:$senderId}),(p2:Person {userId:$receiverId}) CREATE (p1)-[:REQUESTED_TO]->(p2)")
    void addConnectionRequest(Long senderId, Long receiverId);

    @Query("MATCH (p1:Person {userId:$senderId})-[r:REQUESTED_TO]->(p2:Person {userId:$receiverId}) DELETE r")
